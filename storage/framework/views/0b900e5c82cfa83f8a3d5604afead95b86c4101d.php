    ...
    <!-- Navbar Right Menu -->
        <div class="navbar-custom-menu">
            ...
            <?php if((auth()->user()->can('record.view') || auth()->user()->can('record.view_own'))): ?>
                <?php if(config('app.env') != 'demo'): ?>
                    <a title="<?php echo app('translator')->getFromJson('lang_v1.supplier_calendar'); ?>" href="<?php echo e(route('calendar'), false); ?>" data-toggle="tooltip"
            ...
            <button id="btnCalculator" title="<?php echo app('translator')->getFromJson('lang_v1.calculator'); ?>" type="button"
                    class="btn btn-success btn-flat pull-left m-8 hidden-xs btn-sm mt-10 popover-default"
                    data-toggle="popover" data-trigger="click" data-content='<?php echo $__env->make("layouts.partials.calculator", \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>'
            ...
            <?php if(in_array('pos_sale', $enabled_modules)): ?>
                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('sell.create')): ?>
                    <a href="<?php echo e(action('SellPosController@create'), false); ?>" title="<?php echo app('translator')->getFromJson('sale.pos_sale'); ?>"
                       data-toggle="tooltip" data-placement="bottom"
        ...
        <!-- Help Button -->
            <?php if(auth()->user()->hasRole('Admin#' . auth()->user()->business_id)): ?>
                <button type="button" id="start_tour" title="<?php echo app('translator')->getFromJson('lang_v1.application_tour'); ?>" data-toggle="tooltip"
                        ...
                        <li class="user-footer">
                            <div class="pull-left">
                                <a href="<?php echo e(action('UserController@getProfile'), false); ?>"
                                   class="btn btn-default btn-flat"><?php echo app('translator')->getFromJson('lang_v1.profile'); ?></a>
                            </div>
                            <div class="pull-right">
                                <a href="<?php echo e(action('Auth\LoginController@logout'), false); ?>"
                                   class="btn btn-default btn-flat"><?php echo app('translator')->getFromJson('lang_v1.sign_out'); ?></a>
                            </div>
                        </li>
                    </ul>
                </li>
...
</header><?php /**PATH C:\Users\HP\Project\FreshKtm\Freshktm-\resources\views/layouts/partials/header.blade.php ENDPATH**/ ?>